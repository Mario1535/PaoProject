...
mediaEditor::mediaEditor(AbstractMedia::mediaType type, QWidget *parent)
    : QDialog(parent)
...
void mediaEditor::onSaveButtonClicked() {
    ...
    QPixmap picture;
    if (const QPixmap* pixPtr = ui->photoLabel->pixmap()) {
        picture = *pixPtr;  // copia l'immagine se presente
    } else {
        picture = QPixmap();  // immagine vuota
    }
